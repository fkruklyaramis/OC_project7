...
def bruteforce_best_investment(actions, max_budget=500):
    ...
    toutes les combinaisons possibles (approche bruteforce).
    
    Arguments:
        actions (list): Liste des objets Action disponibles
        max_budget (float): Budget maximum disponible (par défaut: 500€)
        
    Returns:
        tuple: (meilleure_combinaison, coût_total, profit_total)
    """
    best_combination = []     # Stocke la meilleure combinaison d'actions trouvée
    best_profit = 0           # Stocke le meilleur profit trouvé
    best_cost = 0             # Stocke le coût associé à la meilleure combinaison
    total_combinations = 0    # Compteur du nombre total de combinaisons testées
...
Pour chaque combinaison, la fonction:
